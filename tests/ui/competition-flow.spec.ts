...
for (const width of [360,390,430]) test(`mobile ${width}: category → example → follow-up without finding a separate submit button`,async({page})=>{
  ...
  await expect(page.getByRole('button',{name:'민원·생활'})).toBeInViewport();
...
test('공식기관 행동 버튼과 답변 바로 아래 글자·음성 재질문',async({page})=>{
 ...
 const drawer=page.getByRole('dialog');await drawer.getByRole('button',{name:'청년·일자리·기타',exact:true}).click();
 ...
 await expect(page.getByRole('button',{name:'기타',exact:true})).toBeInViewport();
});
