...
async function main() {
  ...
  const configuration = createOpenAIAnswerGenerator();
  ...
  const response = await createPublicInformationResponseWithAnswer(
    { query: "노인맞춤돌봄서비스 신청하려면 어떻게 해야 하나요?" },
    () => configuration,
  );
  const elapsedMs = Math.round(performance.now() - startedAt);
  console.log(`응답 시간: ${elapsedMs} ms (검색·OpenAI 요청·검증·조합 포함).`);
  if ("answerGeneration" in response.body && response.body.answerGeneration?.status === "generated") {
    console.log("PASS: 실제 OpenAI 호출 1회, 응답 파싱·안전 검증·답변 조합 성공.");
  } else {
    console.log("FAIL: 실제 생성 확인 실패. 설정/모델 접근 권한/네트워크/8초 제한을 확인하세요. 추가 호출 없음.");
    process.exitCode = 1;
  }
}
