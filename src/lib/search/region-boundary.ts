...
const NON_REGIONS = new Set([
  ...
  "검정고시", "고시", "전시", "응시", "재응시", "재발급시", "발급시", "야간개시", "분리배출시",
]);
...
const OUTSIDE_CITY = new RegExp(`(?:^|[^가-힣])(?:수원|부천|용인|고양|안양|과천|안산|화성|광명|남양주|의정부|평택|오산|시흥|군포|하남|양주|구리|이천|여주|파주|김포|포천|춘천|원주|강릉|청주|천안|아산|전주|군산|익산|목포|여수|순천|경주|포항|구미|창원|김해|양산|진주|통영)${REGION_END}`);
...
export function regionBoundaryGuard(question: string): IntentRoute | null {
  ...
  const outside = OUTSIDE_METRO.test(query) || OUTSIDE_CITY.test(query) || [...query.matchAll(ADMIN_REGION)]
